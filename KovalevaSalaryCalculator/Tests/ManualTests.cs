...
namespace KovalevaSalaryCalculator.Tests
{
    public class ManualTests
    {
        public static void Run()
        {
            ...
            var calc1 = salaryService.CalculateSalary(emp1, 0, 20, 20, new DateTime(2025, 1, 31), false, settings, history);
            Console.WriteLine($"Test 1 (2800 Deduction): NDFL={calc1.NDFL} (Expected 936)");

            // 2. Progressive NDFL (18% tier)
            // Threshold: 5,000,000.
            // We simulate history to reach 4.95M taxable base.
            var emp2 = new Employee { Id = Guid.NewGuid(), Name = "Pro Test", BaseSalary = 100000, ChildrenCount = 0 };
            history.Add(new SalaryCalculation {
                EmployeeId = emp2.Id,
                Period = new DateTime(2025, 1, 31),
                GrossSalary = 4950000,
                TaxableBase = 4950000,
                IsAdvance = false
            });

            var calc2 = salaryService.CalculateSalary(emp2, 0, 20, 20, new DateTime(2025, 2, 28), false, settings, history);
            // 50,000 @ 15% = 7500.
            // 50,000 @ 18% = 9000.
            // Total: 16500.
            ...
            var adv3 = salaryService.CalculateSalary(emp3, 0, 10, 20, new DateTime(2025, 3, 31), true, settings, history);
            history.Add(adv3);
            var final3 = salaryService.CalculateSalary(emp3, 0, 20, 20, new DateTime(2025, 3, 31), false, settings, history);
            ...
            var calc4_1 = salaryService.CalculateSalary(emp4, 0, 20, 20, new DateTime(2025, 4, 30), false, settings, history);
            history.Add(calc4_1);

            emp4.BaseSalary = 200000;
            var calc4_2 = salaryService.CalculateSalary(emp4, 0, 20, 20, new DateTime(2025, 5, 31), false, settings, history);
